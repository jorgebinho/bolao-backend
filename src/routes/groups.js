...
router.delete("/:id/members/:userId", async (req, res) => {
	try {
		...
		if (
			!membership ||
			(membership.role !== "OWNER" && req.user.role !== "ADMIN")
		) {
			return res
				.status(403)
				.json({ error: "Você não pode gerenciar este grupo." });
		}

		const target = await prisma.groupMember.findUnique({
			where: {
				userId_groupId: { userId: req.params.userId, groupId: req.params.id },
			},
			include: { group: true },
		});
		...
		if (target.group.isGlobal)
			return res
				.status(400)
				.json({ error: "Não é possível remover do grupo global." });
		if (target.role === "OWNER")
			return res
				.status(400)
				.json({ error: "Não é possível remover o dono do grupo." });
