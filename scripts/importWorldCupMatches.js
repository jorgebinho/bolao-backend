...
function readCsv(fileNameOrPath) {
	const filePath = path.isAbsolute(fileNameOrPath) ? fileNameOrPath : path.join(ARCHIVE_DIR, fileNameOrPath)
...
function teamNameFromMatch(row, teamsById, side, options = { translatePlaceholder: true }) {
	...
	return (options.translatePlaceholder ? translatePlaceholder(fallback) : fallback) || 'A definir'
}
...
function legacyMatchKeyFromRow(row, legacyTeamsById) {
	const legacyMatch = {
		homeTeam: teamNameFromMatch(row, legacyTeamsById, 'home', { translatePlaceholder: false }),
		awayTeam: teamNameFromMatch(row, legacyTeamsById, 'away', { translatePlaceholder: false }),
...
async function main() {
	...
	const existingByKey = new Map(existing.map((match) => [matchKey(match), match]))
	const existingByLegacyKey = new Map(existing.map((match) => [matchKey(match), match]))
	const missing = payload.filter((match) => (
		!existingKeys.has(matchKey(match)) && !existingByLegacyKey.has(match.legacyKey)
	))
	const updates = payload
		.map((match) => ({
			match,
			existing: existingByKey.get(matchKey(match)) || existingByLegacyKey.get(match.legacyKey),
		}))
